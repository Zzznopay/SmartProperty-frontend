...
import {
  addGreeneryCheck,
  deleteGreeneryCheck,
  type GreeneryCheck as GreeneryCheckType,
  listGreeneryChecks,
  updateGreeneryCheck,
} from '@/services/smart-property/operation/greeneryCheck';
...
type FormProps = {
  ...
  reload?: () => void;
  values?: GreeneryCheckType;
};

const GreeneryCheckForm: React.FC<FormProps> = ({
  trigger,
  reload,
  values,
}) => {
  ...
  return (
    <ModalForm<GreeneryCheckType>
      title={isEdit ? '编辑绿化检查' : '新建绿化检查'}
      trigger={trigger as React.ReactElement<unknown>}
      initialValues={values}
      onFinish={async (vals) => {
        try {
          if (isEdit && values?.id) {
            await updateGreeneryCheck(values.id, vals);
          } else {
            await addGreeneryCheck(vals);
          }
          reload?.();
          return true;
        } catch {
...
const GreeneryCheckList: React.FC = () => {
  const actionRef = useRef<ActionType | null>(null);
  const { message } = App.useApp();

  const cols: ProColumns<GreeneryCheckType>[] = [
    ...
    },
    {
      ...
      render: (_, record) => [
        ...
        <Popconfirm
          ...
          onConfirm={async () => {
            try {
              await deleteGreeneryCheck(record.id!);
  ...
  return (
    <PageContainer>
      <ProTable<GreeneryCheckType>
        actionRef={actionRef}
        rowKey="id"
        columns={cols}
        request={async (params) => {
          const resp = await listGreeneryChecks({
        ...
        toolBarRender={() => [
          <GreeneryCheckForm
            key="create"
            trigger={<Button type="primary">新建绿化检查</Button>}
            reload={() => actionRef.current?.reload()}
          />,
        ]}
      />
...
export default GreeneryCheckList;
